...
def get_description():
    return 'This module uses bruteforce of tumblr.com subdomain to find company blogs'
...
def run(words):
    ...
    urls = compile_subdomain('tumblr.com', words)
    log.debug('Run requests...')
    loop = asyncio.get_event_loop()
    responses = loop.run_until_complete(async_requests(urls))
    founded_projects = []
    for r in responses:
        # Tumblr returns 404 for non-existent blogs, 200/302 for existing
