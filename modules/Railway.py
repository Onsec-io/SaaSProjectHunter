...
def get_description():
    return 'This module uses bruteforce of up.railway.app subdomain to find deployed applications'


def wordslist_for_check_module():
    return {
        'real': ['plausible', 'brilliants', 'lifeos'],
...
def run(words):
    ...
    urls = compile_subdomain('up.railway.app', words)
    log.debug('Run requests...')
    loop = asyncio.get_event_loop()
    responses = loop.run_until_complete(async_requests(urls))
    founded_projects = []
    for r in responses:
        # Railway returns 404 for non-existent apps
