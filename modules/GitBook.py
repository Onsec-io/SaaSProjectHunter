...
def get_description():
    return 'This module uses bruteforce of gitbook.io subdomain to find documentation sites'


def wordslist_for_check_module():
    return {
        'real': ['quii', 'bellingcat', 'appsecexplained'],
...
def run(words):
    ...
    urls = compile_subdomain('gitbook.io', words)
    log.debug('Run requests...')
    loop = asyncio.get_event_loop()
    responses = loop.run_until_complete(async_requests(urls))
    founded_projects = []
    for r in responses:
        # GitBook returns 404 for non-existent docs, 307 redirect for existing
