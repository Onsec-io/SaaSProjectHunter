...
def get_description():
    return 'This module uses bruteforce of softr.app subdomain to find no-code apps and portals'


def wordslist_for_check_module():
    return {
        'real': ['canresearch', 'thegallery', 'members'],
...
def run(words):
    ...
    urls = compile_subdomain('softr.app', words)
    log.debug('Run requests...')
    loop = asyncio.get_event_loop()
    responses = loop.run_until_complete(async_requests(urls))
    founded_projects = []
    for r in responses:
        # Softr returns 404 for non-existent apps
