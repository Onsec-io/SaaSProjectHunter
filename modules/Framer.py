...
def get_description():
    return 'This module uses bruteforce of framer.website subdomain to find Framer sites'


def wordslist_for_check_module():
    return {
        'real': ['lyniq', 'sections', 'praxis'],
...
def run(words):
    ...
    urls = compile_subdomain('framer.website', words)
    log.debug('Run requests...')
    loop = asyncio.get_event_loop()
    responses = loop.run_until_complete(async_requests(urls))
    founded_projects = []
    for r in responses:
        # Framer returns 404 for non-existent sites
